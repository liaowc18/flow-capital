...
app.post('/api/valuation', async (c) => {
  try {
    ...
    const { name, industry, revenue, cost, dailyFlow, dailyCost, netProfit, refundRate, growthRate, turnoverDays, debtRatio, employeeCount, operatingYears, attachments } = body

    // 处理附件 — 提取文本内容用于 LLM 分析
    const fileTexts: string[] = []
    const fileMetadata: { name: string; type: string; size: number }[] = []
    if (Array.isArray(attachments)) {
      for (const att of attachments) {
        fileMetadata.push({ name: att.name, type: att.type, size: att.size })
        if (att.textContent) {
          // CSV / TXT / JSON 等文本文件：截取前 8000 字符防止 prompt 溢出
          fileTexts.push(`【文件：${att.name}】\n${att.textContent.slice(0, 8000)}`)
        }
      }
    }
    ...
    if (apiKey) {
      try {
        result = await callLLM(apiKey, baseUrl, { name, industry, annualRevenue, annualCost, calcNetProfit, profitMargin, dailyCashFlow, dailyCost, refundRate, growthRate, turnoverDays, debtRatio, employeeCount, operatingYears, fileTexts, fileMetadata })
      } catch (e: any) {
    ...
    if (fileMetadata.length > 0) {
      result._attachments = fileMetadata.map((f: any) => ({ name: f.name, type: f.type, size: f.size }))
    }
...
请按六维体检+基因分型+估值定价+造局参数+评估报告的完整流程输出，所有物理结论都翻译成行业话。${data.fileTexts && data.fileTexts.length > 0 ? `

📎 用户上传了 ${data.fileMetadata.length} 份辅助材料，以下是可读取的文本内容，请结合这些数据进行更深入的分析推演：
${data.fileTexts.join('\n\n')}

如果文件中包含具体的月度/季度数据，请在分析中体现趋势判断和异常检测。` : data.fileMetadata && data.fileMetadata.length > 0 ? `

📎 用户上传了 ${data.fileMetadata.length} 份辅助材料：${data.fileMetadata.map((f: any) => f.name).join('、')}。这些文件为二进制格式（PDF/图片等），暂无法在线解析，但请在风控建议中注明已收到材料、建议人工核验。` : ''}`
